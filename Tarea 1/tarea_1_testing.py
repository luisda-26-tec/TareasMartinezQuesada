...
def test_casos_error_operation_selector():
    ...
    estado, res1 = tarea_1_example_solution.operation_selector(7, 7.1, "+") #Debe dar un error -50 al tener un float como segundo operando
    assert estado == -50
    assert res1 is None
    estado, res1 = tarea_1_example_solution.operation_selector(12, "s", "*") #Debe dar error -50 al tener un string como operando
    assert estado == -50
    assert res1 is None
    estado, res1 = tarea_1_example_solution.operation_selector(True, 7, "-") #Debe dar error -50 al tener un booleano en operando
    assert estado == -50
    assert res1 is None
    estado, res1 = tarea_1_example_solution.operation_selector(None, 77, "&")#Debe dar error -50 al tener booleano como operando
    assert estado == -50
    assert res1 is None
